...
- Finished quests still turn in, including a single reward. New quests and party-shared quests stay on screen.
- Existing characters had quest accept turned off once. A new character does not accept quests.
- Gossip clicks only a lone vendor, binder, flight master, trainer, bank, or inn. A story line stays up. A flight master opens the map and does not pick the destination.
...
- A poor item on the keep list is not sold with the rest of the junk.
- Mail stops retrying an attachment that does not fit.
- A quest with no reward choice is handed in without passing a choice index.
- Talking heads are hooked when that frame loads, and Shift still leaves one up.
...
- Faster autoloot when Auto Loot is enabled
- Sell junk and repair at merchants
- Take mailbox gold and attachments, leaving COD mail
- Accept and turn in quests, leaving multi-item rewards
- Click a gossip window that has only one option
- Release in battlegrounds, accept resurrections, skip combat res
- Skip cinematics and talking heads
- Dismount or stand when the game requires it
- Confirm poor-quality item deletes
